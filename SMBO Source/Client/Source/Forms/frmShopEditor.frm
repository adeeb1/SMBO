...
Public Sub LoadShopItemData(ShopNum As Integer)
    ...
    For i = 1 To 25
        ...
        ShopItemList(i).currencyItem = Shop(ShopNum).ShopItem(i).currencyItem
    Next i
End Sub

' Adds the specified item to the shop list and temporary array
Public Sub AddShopItem(ByVal itemN As Integer, ByVal prc As Currency, ByVal cItem As Integer, Optional ByVal AMT As Integer = 0)
    Dim itemStr As String
    
    If lstItems.ListCount + 1 > MAX_SHOP_ITEMS Then
       Call MsgBox("The shop already holds its maximum number of items!", vbExclamation)
       Exit Sub
    End If
    ...
    ' Doesn't add shop item if shop is full
        ...
        ShopItemList(lstItems.ListCount).currencyItem = cItem
    End If
End Sub

' Edits the shop item in the list and array
Public Sub EditShopItem(ByVal Index As Integer, ByVal itemN As Integer, ByVal prc As Currency, ByVal cItem As Integer, Optional ByVal AMT As Integer = 0)
    Dim itemStr As String
    ...
    If itemN > 0 And itemN <= MAX_ITEMS Then
        If Index >= 0 And Index <= MAX_SHOP_ITEMS Then
            ...
            ShopItemList(Index + 1).currencyItem = cItem
        End If
    End If
End Sub
    ...
    Else
        ...
        Else
            ...
            If lstItems.ListIndex >= 0 Then
                Call EditShopItem(lstItems.ListIndex, cmbItemList.ListIndex + 1, Val(txtPrice.Text), currencyItem, Val(txtNumber.Text))
            End If
        End If
...
Dim Index As Integer
  Index = lstItems.ListIndex
    If lstItems.ListIndex >= 0 Then
        ' Remove the item
        Call lstItems.RemoveItem(Index)
      For i = (Index + 1) To MAX_SHOP_ITEMS - 1
        ShopItemList(i).Amount = ShopItemList(i + 1).Amount
        ShopItemList(i).ItemNum = ShopItemList(i + 1).ItemNum
        ShopItemList(i).Price = ShopItemList(i + 1).Price
        ShopItemList(i).currencyItem = ShopItemList(i + 1).currencyItem
      Next i
...
Public Function GetShopItemAmt(ByVal Item As Integer) As Currency
...
Public Function GetShopItemPrice(ByVal Item As Integer) As Currency
    If Item > 0 And Item < lstItems.ListCount + 1 Then
    ...
    End If
End Function
' Returns currency item of shopitem in temp array
Public Function GetShopCurrencyItem(ByVal Item As Integer) As Integer
    If Item > 0 And Item < lstItems.ListCount + 1 Then
        GetShopCurrencyItem = ShopItemList(Item).currencyItem
    ElseIf Item < 26 Then
        GetShopCurrencyItem = 0
    End If
End Function
